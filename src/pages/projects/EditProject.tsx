...
const departments = ["IT", "HR", "Finance", "Marketing", "Digital Marketing", "Sales", "Operations", "Design", "Engineering", "Development"];
...
const statusColors: Record<WorkStatus, string> = {
  ...
  Running: "border-orange-500 bg-orange-500",
  Pending: "border-amber-500 bg-amber-500",
  "Not Started": "border-gray-400 bg-gray-400",
...
const prefilled: FormData = {
  projectId: "PROJ-001",
  projectTitle: "WebNxt HRM - Admin Dashboard",
  ...
  team: ["e1", "e2"],
  workStatus: "Active",
  projectDescription: "A modern material design based admin dashboard for WebNxt HRM system...",
  budget: "15000",
...
const EditProject = () => {
  ...
  const handleUpdate = () => { if (validate()) alert("Project updated successfully!"); };
  const handleCancel = () => { setForm({ ...prefilled }); setErrors({}); setUploadedFiles([]); };
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <h1 className="text-xl font-bold text-gray-900">Edit Projects</h1>

      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {/* Card Header */}
        <div className="px-7 py-5 border-b border-gray-200 bg-gradient-to-r from-amber-50/60 to-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[16px] font-extrabold text-gray-900">Edit Projects</h2>
              <p className="text-[12px] text-gray-600 font-medium">Update the project details below</p>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <span className="px-3 py-1 bg-blue-50 border border-blue-200 rounded-full text-[11px] font-bold text-blue-700">
                {form.projectId}
              </span>
              <span className={`px-3 py-1 rounded-full text-[11px] font-bold text-white ${statusColors[form.workStatus].split(" ")[1]}`}>
                {form.workStatus}
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              {workStatuses.map(status => {
                ...
                return (
                  <label key={status} className="inline-flex items-center gap-2 cursor-pointer group" onClick={() => set("workStatus", status)}>
                    <span className={`w-[18px] h-[18px] rounded-full border-2 flex items-center justify-center transition-all ${
                      active ? color : "border-gray-300 group-hover:border-gray-400"
                    }`}>
                      {active && <span className="w-2 h-2 rounded-full bg-white" />}
                    </span>
                    <span className={`text-[13px] font-semibold transition-colors ${active ? "text-gray-900" : "text-gray-600 group-hover:text-gray-800"}`}>
                      {status}
                    </span>
                  </label>
                );
            ...
            return (
              <button key={opt} type="button"
                ...
                className={`w-full text-left px-4 py-2.5 text-[13px] font-semibold transition-all duration-150 ${
                  sel ? "bg-blue-50 text-blue-700 border-l-[3px] border-blue-500 pl-[13px]" : "text-gray-800 hover:bg-blue-50/50 hover:text-blue-600 hover:pl-5 border-l-[3px] border-transparent"
                }`}>
          ...
          <div ref={ref} className="relative">
            ...
            {open && (
              <div className="absolute z-50 left-0 top-full mt-1.5 bg-white rounded-xl border border-gray-200 shadow-xl py-1.5 w-44 animate-fade-in">
                {currencies.map(c => {
                  const sel = currency.label === c.label;
                  return (
                    <button key={c.label} type="button" onClick={() => { onCurrencyChange(c); setOpen(false); }}
                      className={`w-full flex items-center gap-2.5 px-3.5 py-2.5 text-[13px] font-medium transition-all duration-150 ${
                        sel ? "bg-blue-50 text-blue-700 border-l-[3px] border-blue-500 pl-[11px]" : "text-gray-700 hover:bg-blue-50/50 hover:text-blue-600 border-l-[3px] border-transparent"
                      }`}>
                      <span className="text-[16px]">{c.flag}</span>
                      <span className="font-bold">{c.symbol}</span>
                      <span className="text-gray-400 font-normal">{c.label}</span>
                      {sel && <Check className="w-3.5 h-3.5 ml-auto text-blue-500" />}
                    </button>
                  );
                })}
              </div>
